	...
	.component("forum", {
		...
		controller: function(topicService, postService, tagService, $filter, $location, $routeParams){
			...
			var getAllTopics = function(){
				topicService.index()
				.then(function(resp){
					console.log("reached inside of getAllTopics");
			...
			var getNonCareerTopics = function(){
				console.log("inside getNonCareerTopics");
				var tempList = vm.allTopics;
				console.log(tempList);
				console.log(vm.allTopics);
				tempList.forEach(function(value, index, array){
					var tag = value.tag;
					console.log(tag);
					if(!(value.tag.type == 'Resume' || value.tag.type == 'Interview' || value.tag.type == 'CoverLetter')) {
						console.log('inside if statement')
						vm.currentTopics.push(value);
					}
				});
